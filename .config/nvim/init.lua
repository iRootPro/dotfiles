...
-- Keep normal-mode shortcuts usable when the keyboard layout is Russian.
...
local function file_path()
	...
	local root = vim.fn.systemlist({ "git", "rev-parse", "--show-toplevel" })[1]
	if vim.v.shell_error == 0 and root and root ~= "" and vim.startswith(full, root .. "/") then
...
vim.opt.statusline = ""
	...
	.. " %{v:lua.file_path()}" -- короткий путь файла
	.. "%m%r" -- модификаторы
